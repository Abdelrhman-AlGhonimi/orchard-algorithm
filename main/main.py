...
def objective_function(solution, data):
    ...
    # Penalty weights (positive values)
    emergency_weight = 5.0
    idle_weight = 2.0
    overlap_weight = 5.0  # Reduced from 10.0
    usage_weight = 1.0
    surgeon_weight = 5.0  # Reduced from 10.0
    max_possible_score = 1000  # Maximum score for perfect schedule
    overlap_threshold = 20  # Maximum allowed overlap in minutes

    # Calculate penalties
    penalties = {
        'emergency_wait': emergency_wait * emergency_weight,
        'idle_time': idle_time * idle_weight,
        'overlap': min(overlap, overlap_threshold) * overlap_weight,
        'surgeon_conflict': min(surgeon_conflict, overlap_threshold) * surgeon_weight,
        'room_usage': abs(room_usage - (len(operations) * 30)) * usage_weight
    }
    ...
    score = max_possible_score - total_penalty
    
    # Adjust score based on conflicts
    if overlap > overlap_threshold:
        score *= 0.5  # Only reduce score by 50% if overlap exceeds threshold
    if surgeon_conflict > overlap_threshold:
        score *= 0.5  # Only reduce score by 50% if surgeon conflict exceeds threshold
...
def initialize_seedling(data):
    ...
    surgeon_schedule = {surgeon: [] for surgeon in data['surgeons']}  # Track surgeon's schedule
    ...
    for op in operations_sorted:
        ...
        # Find a room and start time that doesn't conflict
        room = None
        start_time = None
        
        # Try all rooms to find a valid slot
        for r in rooms:
            # Check if room has enough time left
            if room_usage[r] + duration <= day_duration:
                # Get all operations in this room
                room_ops = [(start, end) for start, end in surgeon_schedule[surgeon]]
                
                # Find a valid start time that doesn't conflict
                current_time = room_start_times[r]
                while current_time + duration <= day_duration:
                    conflicts = False
                    
                    # Check for conflicts with existing operations in this room
                    for start, end in room_ops:
                        if (current_time < end and current_time + duration > start):
                            conflicts = True
                            break
                    
                    # Check for conflicts with other surgeons in this room
                    for s in surgeon_rooms:
                        if s != surgeon:
                            for op_start, op_end in surgeon_schedule[s]:
                                if (current_time < op_end and current_time + duration > op_start):
                                    conflicts = True
                                    break
                            if conflicts:
                                break
                    ...
                    if not conflicts:
                        # Calculate room load (number of operations)
                        room_load = len(surgeon_rooms.get(r, []))
                        
                        # Prefer rooms with fewer operations
                        if room is None or room_load < len(surgeon_rooms.get(room, [])):
                            room = r
        ...
        # If no valid slot found in any room, use the least used room
        if room is None:
            # Find room with minimum usage that can fit the operation
            valid_rooms = [(r, room_usage[r]) for r in rooms 
                           if room_usage[r] + duration <= day_duration]
            
            if valid_rooms:
                # Sort by usage and then by number of operations
                room, _ = min(valid_rooms, key=lambda x: (x[1], len(surgeon_rooms.get(x[0], []))))
                # Find latest end time for this surgeon in any room
                latest_end = max((op[1] for op in surgeon_schedule[surgeon]), default=0)
                # Start after the latest end time
                start_time = latest_end + 1  # Add 1 minute gap
            else:
                # If no room can fit the operation, use the least used room
                room = min(rooms, key=lambda r: room_usage[r])
                # Start at the end of the day
                start_time = day_duration - duration

        # Update room and surgeon schedules
        room_start_times[room] = start_time + duration
        room_usage[room] += duration
        
        # Add to surgeon's schedule (start, end)
        surgeon_schedule[surgeon].append((start_time, start_time + duration))
        
        # Track room usage by surgeon
        if room not in surgeon_rooms[surgeon]:
