...
==================================================""")
    ...
    for room in rooms:
        print(f"\nRoom {room}:")
        print("-" * 70)
        for op in sorted(room_schedule[room], key=lambda x: int(x['start_time'].split(':')[0]) * 60 + int(x['start_time'].split(':')[1])):
...
if __name__ == "__main__":
    try:
        ...
        if best_solution is not None:
            ...
            for room in rooms:
                room_schedule[room].sort(key=lambda x: int(x['start_time'].split(':')[0]) * 60 + int(x['start_time'].split(':')[1]))
